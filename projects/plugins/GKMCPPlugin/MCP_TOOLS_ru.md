# Справочник инструментов GKmcp

## Обзор

**GKmcp** — это плагин GEDKeeper с поддержкой протокола [MCP](https://modelcontextprotocol.io/),
...
## Основные опции (устарело, было актуально для реализации на базе stdio)

- `--mcp` - опция запуска CLI-приложения в режиме MCP сервера, базовая и обязательная.
- `--pure` - опция использования только инструментов для работы со стандартными GEDCOM структурами и записями.
- `--tde` - опция режима работы `Tool Discovery & Execution`, для предельной экономии токенов системного промта.
- `--rag` - опция включения инструментов Retrieval-Augmented Generation для парсинга исторических документов с архаичной орфографией.
...
## Инструменты RAG (Retrieval-Augmented Generation)

| Инструмент | Описание | Параметры |
|---|---|---|
| `rag_search_examples` | Инструмент для поиска шаблонов парсинга старинных документов | `input_text` (string, обязательный) - Оригинальный текст переписи (архаичная орфография), `century` (string) - Век для фильтрации шаблонов: '17', '18', '19', `min_score` (number) - Минимальный порог косинусного сходства (0.0–1.0), по умолчанию 0.6, `top_k` (integer) - Максимальное количество шаблонов для возврата, по умолчанию 3 |
| `rag_write_pattern` | Инструмент для записи образца текста переписи с эталонным результатом прочтения | `raw_text` (string, обязательный) - Оригинальный текст переписи (архаичная орфография), `corrected_result` (string, обязательный) - Идеальный результат парсинга (JSON/структура), `century` (string, обязательный) - Век: '17', '18', '19', `quality_score` (number) - Пользовательская оценка качества шаблона |
...
2. Нет инструментов для расширенного поиска по датам, местам, событиям или комбинации критериев.
3. Нет инструментов для генерации статистики, построения диаграмм, анализа поколений и т.д.
