...
**GKmcp** is a plugin for GEDKeeper with support for the [MCP](https://modelcontextprotocol.io/) protocol,
enabling LLM clients to interact with genealogical databases through a comprehensive set of tools.

**Launching in LLM clients:** it works if the MCP server within the plugin is running and the local server address is configured in the LM client.

**Protocol:** MCP `2025-06-18`, JSON-RPC 2.0 over Streamable HTTP. No external dependencies.
...
- **LM Studio**: The client restarts a new instance of the MCP server every 1-2 minutes, leading to continuous context resets in the database (applicable only to the old implementation based on stdio).
...
Start working in LM chat with GKmcp enabled by saying,
...
Configuration in **Jan** (`mcp_config.json`) for connection (Streamable HTTP):
