...
internal class LocationListNamesTool : BaseTool
{
    public LocationListNamesTool() : base("location_list_names") { }
...
internal class LocationUpsertNameTool : BaseTool
{
    public LocationUpsertNameTool() : base("location_upsert_name") { }

    public override MCPTool CreateTool()
    {
        return new MCPTool {
            Name = Sign,
            Description = "Add name to location record or update existing. Provide 'name_index' to edit; omit 'name_index' to create. 'location_xref' and 'name' required for new names.",
            InputSchema = new MCPToolInputSchema {
                Properties = new Dictionary<string, MCPToolProperty> {
                    ["location_xref"] = new MCPToolProperty { Type = "string", Description = "XRef identifier of the location (e.g., 'L1')" },
                    ["name_index"] = new MCPToolProperty { Type = "integer", Description = "Zero-based index of the name in the location's name list (omit for new)" },
    ...
    public override List<MCPContent> ExecuteTool(BaseContext baseContext, JsonElement args)
    {
        ...
        if (isEdit) {
            ...
            if (index < 0 || index >= locRec.Names.Count)
                return MCPContent.CreateSimpleContent($"❌ Invalid name index {index} for location '{locationXRef}' (has {locRec.Names.Count} names).");

            var locationName = locRec.Names[index];
            ...
            return MCPContent.CreateSimpleContent($"✅ Name updated for location '{locationXRef}' at index {index}: {nameInfo}");
        } else {
            if (string.IsNullOrEmpty(name))
                return MCPContent.CreateSimpleContent("❌ 'name' required for new location name");

            var locationName = new GDMLocationName();
            locationName.StringValue = name;
            locationName.Abbreviation = shortName ?? string.Empty;
            ...
            baseContext.SetModified();
            int newIndex = locRec.Names.IndexOf(locationName);
            return MCPContent.CreateSimpleContent($"✅ Name added to location '{locationXRef}' at index {newIndex}: '{name}'");
        }
    }
}


internal class LocationDeleteNameTool : BaseTool
{
    public LocationDeleteNameTool() : base("location_delete_name") { }
