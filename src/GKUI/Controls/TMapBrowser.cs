...
namespace GKUI.Controls
{
	public class TMapBrowser : WebBrowser, IDisposable
	{
		...
		public void InitMap()
		{
			this.FMapFile = SysUtils.GetTempDir() + "\\GEDKeeperMap.html";
		...
		public static bool GetInetFile([In] string FileURL, ref Stream Stream)
		{
			...
			try
			{
				...
				TProxy proxy = GKUI.TfmGEDKeeper.Instance.Options.Proxy;
				if (proxy.UseProxy)
				{
							...
							for (int i = 0; i <= num; i++)
							{
								if (Node.ChildNodes[i].Name == "Placemark")
								{
									...
									if (AddressNode != null && PointNode != null)
									{
										...
										{
											pt.Longitude = SysUtils.StrToFloatDef(coord_tokens[0], -1.0);
											pt.Latitude = SysUtils.StrToFloatDef(coord_tokens[1], -1.0);
