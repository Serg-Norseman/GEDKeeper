...
uses
  System.Globalization, System.Threading, VCLStub;

type
  TCalendarConv = class(TObject)
  private
    function _modf(a, b: Double): Double;
    function _modi(a, b: Double): Longint;
    function _if(aCond: Boolean; aThen, aElse: Longint): Longint;
...
      // (AD - Нашей эры, BC - До нашей эры)
      TDateEra = (AD, BC);

    const
      HebrewMonths: array [1..13] of string = (
        'Нисан', 'Ияр', 'Сиван', 'Тамуз', 'Ав', 'Элул', 'Тишрей',
      ...
      IslamicWeekdays: array [0..6] of string = (
        ...
        'Фарвардин', 'Ордибехешт', 'Хордад', 'Тир', 'Мордад', 'Шахривар',
...
        'Чайтра', 'Ваисакха', 'Джанштха', 'Асадха', 'Сравана', 'Бхадра',
        'Азвина', 'Картика', 'Аграхайана', 'Пауза', 'Магха', 'Пхалгуна'
      );
...
        'Бахa', 'Джалaл', 'Джамaл', 'Азамат', 'Нур', 'Рахмат', 'Калимaт', 'Камaл',
        'Асмa', 'Иззат', 'Машиййат', 'Ильм', 'Кудрат', 'Каул', 'Масa’иль',
        'Шараф', 'Султан', 'Мульк', 'Аййaм-и Хa', 'Алa'
      );
    ...
    function date_to_str(aYear, aMonth, aDay: Integer; aEra: TDateEra): string;
  end;
...
function TCalendarConv._modf(a, b: Double): Double;
begin
  Result := a - (b * Floor(a / b));
end;

function TCalendarConv._modi(a, b: Double): Longint;
begin
  Result := Trunc(a - (b * Floor(a / b)));
end;

function TCalendarConv.jwday(j: Double): Longint;
begin
  Result := _modi(Floor((j + 1.5)), 7);
end;

function TCalendarConv._if(aCond: Boolean; aThen, aElse: Longint): Longint;
begin
  if (aCond) then Result := aThen else Result := aElse;
end;

function TCalendarConv.date_to_str(aYear, aMonth, aDay: Integer; aEra: TDateEra): string;
var
...
begin
  ...
  Result := aDay.ToString() + ' ' + DateTimeInfo.AbbreviatedMonthNames[aMonth-1] + ' ' + aYear.ToString();
  if not(aEra = AD) then Result := Result + ' до н.э.';
end;

{================================== Julian day ================================}

//  LEAP_GREGORIAN  --  Is a given year in the Gregorian calendar a leap year ?
function TCalendarConv.leap_gregorian(year: Longint): Boolean;
begin
...
function TCalendarConv.gregorian_to_jd(year, month, day: Longint): Double;
begin
  Result := (GREGORIAN_EPOCH - 1) +
            (365 * (year - 1)) +
            System.Math.floor((year - 1) / 4) +
            (-System.Math.floor((year - 1) / 100)) +
            System.Math.floor((year - 1) / 400) +
            System.Math.floor((((367 * month) - 362) / 12) +
            (_if(month <= 2, 0, _if(leap_gregorian(year), -1, -2))) + day);
end;

//  JD_TO_GREGORIAN  --  Calculate Gregorian calendar date from Julian day
procedure TCalendarConv.jd_to_gregorian(jd: Double; var year, month, day: Longint);
var
...
begin
  ...
  leapadj := (_if(wjd < gregorian_to_jd(year, 3, 1), 0, _if(leap_gregorian(year), 1, 2)));
...
function TCalendarConv.leap_julian(year: Longint): Boolean;
begin
  Result := _modf(year, 4) = _if(year > 0, 0, 3);
end;

function TCalendarConv.julian_to_jd(year, month, day: Longint): Double;
begin
  ...
  Result := ((System.Math.floor((365.25 * (year + 4716))) +
            System.Math.floor((30.6001 * (month + 1))) + day) - 1524.5);
end;

//  JD_TO_JULIAN  --  Calculate Julian calendar date from Julian day
procedure TCalendarConv.jd_to_julian(jd: Double; var year, month, day: Longint);
var
  b, c, d, e: Longint;
begin
  jd := jd + 0.5;
  ...
  month := Floor(_if(e < 14, e - 1, e - 13));
  year := Floor(_if(month > 2, c - 4716, c - 4715));
...
function TCalendarConv.hebrew_leap(year: Longint): Boolean;
begin
  Result := _modf(((year * 7) + 1), 19) < 7;
end;

//  How many months are there in a Hebrew year (12 = normal, 13 = leap)
function TCalendarConv.hebrew_year_months(year: Longint): Longint;
begin
  Result := _if(hebrew_leap(year), 13, 12);
end;

//  Test for delay of start of new year and to avoid
//  Sunday, Wednesday, and Friday as start of the new year.
function TCalendarConv.hebrew_delay_1(year: Longint): Longint;
var
...
function TCalendarConv.hebrew_delay_2(year: Longint): Longint;
var
...
begin
  ...
  Result := (_if((next - present) = 356, 2, (_if((present - last) = 382, 1, 0))));
end;

//  How many days are in a Hebrew year ?
function TCalendarConv.hebrew_year_days(year: Longint): Double;
begin
  Result := hebrew_to_jd(year + 1, 7, 1) - hebrew_to_jd(year, 7, 1);
end;

//  How many days are in a given month of a given year
function TCalendarConv.hebrew_month_days(year, month: Longint): Longint;
begin
...
function TCalendarConv.hebrew_to_jd(year, month, day: Longint): Double;
var
  jd: Double;
  mon, months: Longint;
begin
...
procedure TCalendarConv.jd_to_hebrew(jd: Double; var year, month, day: Longint);
var
...
begin
  ...
  first := _if(jd < hebrew_to_jd(year, 1, 1), 7, 1);
  month := first;
  i := first;
...
function TCalendarConv.leap_islamic(year: Longint): Boolean;
begin
  Result := (((year * 11) + 14) mod 30) < 11;
end;

//  ISLAMIC_TO_JD  --  Determine Julian day from Islamic date
function TCalendarConv.islamic_to_jd(year, month, day: Longint): Double;
begin
...
procedure TCalendarConv.jd_to_islamic(jd: Double; var year, month, day: Longint);
begin
  jd := Floor(jd) + 0.5;
  year := Floor(((30 * (jd - ISLAMIC_EPOCH)) + 10646) / 10631);
  month := System.Math.Min(12, Ceil((jd - (29 + islamic_to_jd(year, 1, 1))) / 29.5) + 1);
...
function TCalendarConv.leap_persian(year: Longint): Boolean;
begin
  Result := ((((((year - _if((year > 0), 474, 473)) mod 2820) + 474) + 38) * 682) mod 2816) < 682;
end;

//  PERSIAN_TO_JD  --  Determine Julian day from Persian date
function TCalendarConv.persian_to_jd(year, month, day: Longint): Double;
var
  epbase, epyear: Double;
begin
  epbase := year - _if((year >= 0), 474, 473);
  epyear := 474 + _modf(epbase, 2820);

  Result := day +
          _if((month <= 7), ((month - 1) * 31), (((month - 1) * 30) + 6)) +
          Floor(((epyear * 682) - 110) / 2816) +
          (epyear - 1) * 365 + Floor(epbase / 2820) * 1029983 + (PERSIAN_EPOCH - 1);
end;

//  JD_TO_PERSIAN  --  Calculate Persian date from Julian day
procedure TCalendarConv.jd_to_persian(jd: Double; var year, month, day: Longint);
var
  depoch, yday: Double;
  cycle, cyear, ycycle, aux1, aux2: Longint;
begin
  jd := System.Math.floor(jd) + 0.5;
  ...
  month := _if((yday <= 186), Ceil(yday / 31), Ceil((yday - 6) / 30));
...
function TCalendarConv.indian_civil_to_jd(year, month, day: Longint): Double;
var
...
begin
  ...
  start := gregorian_to_jd(gyear, 3, _if(leap, 21, 22));
  Caitra := _if(leap, 31, 30);
...
procedure TCalendarConv.jd_to_indian_civil(jd: Double; var year, month, day: Longint);
var
...
begin
  ...
  Caitra := _if(leap, 31, 30);          // Days in Caitra this year
...
function TCalendarConv.bahai_to_jd(major, cycle, year, month, day: Longint): Double;
var
  gy, by, dummy: Integer;
begin
  ...
  Result := gregorian_to_jd(gy, 3, 20) + (19 * (month - 1)) +
         _if((month <> 20), 0, _if(leap_gregorian(gy + 1), -14, -15)) + day;
end;

//  JD_TO_BAHAI  --  Calculate Bahai date from Julian day
procedure TCalendarConv.jd_to_bahai(jd: Double; var major, cycle, year, month, day: Longint);
var
...
begin
  ...
  bys := gy - (bstarty + _if(((gregorian_to_jd(gy, 1, 1) <= jd) and (jd <= gregorian_to_jd(gy, 3, 20))), 1, 0));
  major := Floor(bys / 361) + 1;
  cycle := Floor(_modf(bys, 361) / 19) + 1;
  year := _modi(bys, 19) + 1;
  days := jd - bahai_to_jd(major, cycle, year, 1, 1);
  bld := bahai_to_jd(major, cycle, year, 20, 1);
  month := _if((jd >= bld), 20, (Floor(days / 19) + 1));
