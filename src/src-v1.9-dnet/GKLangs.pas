unit GKLangs; {trans:fin}

interface

uses
  System.IO, System.Text, VCLStub, GKUtils;

type
  TLangID = VCLStub.LANGID;
...
type
  ...
  LSID_Backward                 = 382, // checkit
  LSID_Forward                  = 383, // checkit
  LSID_Prev                     = 384,
  LSID_Next                     = 385,
...
type
  TLangMan = class(TObject)
  strict private
    class function GetLStr(Index: LSID): string; static;
    function GetSample(Index: LSID): string;
  public
    class property {LS}List[Index: LSID]: string read GetLStr; //default;

    property Sample[Index: LSID]: string read GetSample; default;

    class function LoadFromFile(const aFileName: string): Boolean;
    class procedure DefInit();
    class procedure SaveDefaultLanguage();
  end; 
...
class procedure TLangMan.DefInit();
var
  id: LSID;
begin
  for id := LSID_First to LSID_Last do LSList[id] := LSDefList[id];
end;

class function TLangMan.GetLStr(Index: LSID): string;
begin
  Result := LSList[Index];
end;

function TLangMan.GetSample(Index: LSID): string;
begin

end;

class function TLangMan.LoadFromFile(const aFileName: string): Boolean;
var
...
begin
  if (System.IO.File.Exists(aFileName)) then begin
    lng_file := System.IO.StreamReader.Create(aFileName, Encoding.UTF8);
    try
      lng_file.ReadLine(); // skip first line
      i := 0;
      while not(lng_file.Peek = -1) do begin
        st := lng_file.ReadLine();
        Inc(i);
        LSList[LSID(i)] := st.Trim();
      end;
...
class procedure TLangMan.SaveDefaultLanguage();
var
  lf: System.IO.StreamWriter;
  i: LSID;
begin
  lf := System.IO.StreamWriter.Create(TGKUtils.GetAppPath() + 'langs\russian.sample', False, Encoding.UTF8);
  try
    lf.WriteLine(';'+Int32(LSDefCode).ToString()+','+LSDefName);
    for i := LSID_First to LSID_Last do
      lf.WriteLine(LSDefList[i]);
  finally
    lf.Close;
  end;
end;
