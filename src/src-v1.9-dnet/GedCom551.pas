...
uses
  System.IO, System.Diagnostics, System.Globalization, System.Threading,
  System.Text, VCLStub;
...
type
  ...
  public
    type
      TGEDCOMRestriction = (rnNone, rnConfidential, rnLocked, rnPrivacy);

      TGEDCOMCharacterSet = (csASCII, csANSEL, csUNICODE, csUTF8);

      TGEDCOMSex = (svNone, svMale, svFemale, svUndetermined);

      TGEDCOMSubList = (stNotes, stSource, stMultimedia);
      TGEDCOMLists = TEnumSet;
    ...
    const
      ATagHasInvalidType = 'The tag {0} is of type {1}, but type {2} was expected';
      EMaxPhoneNumbers = 'The maximum number of phone numbers is {0}';
      EMaxEmailAddresses = 'The maximum number of email addresses is {0}';
      EMaxFaxNumbers = 'The maximum number of fax numbers is {0}';
      EMaxWebPages = 'The maximum number of web page addresses is {0}';
      EMaxLanguages = 'The maximum number of languages is {0}';

      SNotAValidMonth = 'The string {0} is not a valid month identifier';
      SNotAValidFrenchMonth = 'The string {0} is not a valid French month identifier';
      SNotAValidHebrewMonth = 'The string {0} is not a valid Hebrew month identifier';
      SNotAValidNumber = 'The string {0} doesn''t start with a valid number';
      SXRefNotTerminated = 'The string {0} contains an unterminated XRef pointer';
      SXRefExpected = 'The string {0} is expected to start with an XRef pointer';
  ...
  public
    ...
    class function CleanXRef(const XRef: string): string;
    class function EncloseXRef(XRef: string): string;

    class function ExtractNumber(const S: string; var N: Integer;
  ...
  strict private
    type
      TGEDCOMState = (osLoading, osReady);
  ...
  public
    ...
    procedure LoadFromStream(AStream: System.IO.StreamReader);
    procedure SaveToStream(AStream: System.IO.StreamWriter);

    procedure SaveHeaderToStream(AStream: System.IO.StreamWriter);
    procedure SaveFooterToStream(AStream: System.IO.StreamWriter);
  ...
  public
    ...
    procedure SaveToStream(AStream: System.IO.StreamWriter); virtual;
  ...
  strict private
    ...
    const
      TagBaseSize = 26;
      TagBase: array [0..TagBaseSize-1] of TTagPropsRec = (
        (Name: ''; EmptySkip: False),

        (Name: 'ADDR'; EmptySkip: True),
        (Name: 'AGNC'; EmptySkip: True),
        (Name: 'AUTH'; EmptySkip: True),
        (Name: 'CAUS'; EmptySkip: True),
        (Name: 'CHAN'; EmptySkip: True),
        (Name: 'CITY'; EmptySkip: True),
        (Name: 'CTRY'; EmptySkip: True),
        (Name: 'DATE'; EmptySkip: True),
        (Name: 'PAGE'; EmptySkip: True),
        (Name: 'PLAC'; EmptySkip: True),
        (Name: 'POST'; EmptySkip: True),
        (Name: 'PUBL'; EmptySkip: True),
        (Name: 'RESN'; EmptySkip: True),
        (Name: 'STAE'; EmptySkip: True),
        (Name: 'TEXT'; EmptySkip: True),
        (Name: 'TIME'; EmptySkip: True),
        (Name: 'TYPE'; EmptySkip: True),
        (Name: 'SUBM'; EmptySkip: True),

        (Name: 'NPFX'; EmptySkip: True),
        (Name: 'GIVN'; EmptySkip: True),
        (Name: 'NICK'; EmptySkip: True),
        (Name: 'SPFX'; EmptySkip: True),
        (Name: 'SURN'; EmptySkip: True),
        (Name: 'NSFX'; EmptySkip: True),

        (Name: '_LOC'; EmptySkip: True)
      );

    function GetTagProps(const aName: string): TTagPropsRec;
  private
    FLevel: Integer;
    FOwner: TGEDCOMObject;
    FName: string;
  ...
  protected
    ...
    function IsEmptySkip(): Boolean;
    procedure SetLevel(Value: Integer);
    procedure SetStringValue(const S: string); virtual;

    procedure SaveTagToStream(AStream: System.IO.StreamWriter; ATag: TGEDCOMTag); overload; virtual;
    procedure SaveTagToStream(AStream: System.IO.StreamWriter; const ATag: string); overload; virtual;
    procedure SaveTagsToStream(AStream: System.IO.StreamWriter; const ATagSorting: array of string); virtual;
    procedure SaveValueToStream(AStream: System.IO.StreamWriter); virtual;
  ...
  public
    ...
    procedure SetTagStrings(ATag: TGEDCOMCustomTag; Value: TStrings); overload;
    procedure SetTagStrings(ATag: TGEDCOMCustomTag; Value: array of string); overload;
  ...
  private
    FLists: TGEDCOMObject.TGEDCOMLists;
    FNotes: TGEDCOMList;
  ...
  protected
    ...
    procedure SetLists(ALists: TGEDCOMObject.TGEDCOMLists);
  public
    destructor Destroy; override;
    ...
    procedure SaveToStream(AStream: System.IO.StreamWriter); override;
  ...
  protected
    ...
    procedure SaveValueToStream(AStream: System.IO.StreamWriter); override;
  ...
  private
    ...
    function GetCharacterSet: TGEDCOMObject.TGEDCOMCharacterSet;
    procedure SetCharacterSet(const Value: TGEDCOMObject.TGEDCOMCharacterSet);
    function GetNotes: TStrings;
    procedure SetNotes(const Value: TStrings);
    function GetTransmissionDateTime: DateTime;
    procedure SetTransmissionDateTime(const Value: DateTime);
  protected
    function AddSubTag(AParent: TGEDCOMCustomTag; const ATag: string;
  ...
  public
    ...
    property CharacterSet: TGEDCOMObject.TGEDCOMCharacterSet read GetCharacterSet write SetCharacterSet;
    ...
    property TransmissionDateTime: DateTime read GetTransmissionDateTime write SetTransmissionDateTime;
  end;
  ...
  public
    ...
    procedure MoveTo(aToRecord: TGEDCOMRecord; aClearDest: Boolean = False); virtual;
    procedure Pack(); override;
    procedure ReplaceXRefs(aMap: TXRefReplaceMap); override;
    procedure ResetOwner(AOwner: TGEDCOMObject); override;
    procedure SaveToStream(AStream: System.IO.StreamWriter); override;
  ...
  private
    ...
    function GetRestriction: TGEDCOMCustomTag.TGEDCOMRestriction;
    procedure SetRestriction(const Value: TGEDCOMObject.TGEDCOMRestriction);
    function GetPlace: TGEDCOMPlace;
  protected
    procedure CreateObj(AOwner, AParent: TGEDCOMObject); override;
  public
    ...
    property Restriction: TGEDCOMObject.TGEDCOMRestriction read GetRestriction write SetRestriction;
  ...
  private
    function GetDate: TGEDCOMDateExact;
    function GetTime: TGEDCOMTime;
    function GetChangeDateTime: DateTime;
    procedure SetChangeDateTime(const Value: DateTime);
  ...
  public
    ...
    property ChangeDateTime: DateTime read GetChangeDateTime write SetChangeDateTime;
  ...
  private
    ...
    function GetStringTag(Index: Integer): string;
    procedure SetStringTag(Index: Integer; const Value: string);
    function GetRestriction: TGEDCOMObject.TGEDCOMRestriction;
    procedure SetRestriction(const Value: TGEDCOMObject.TGEDCOMRestriction);
  ...
  public
    ...
    procedure ChildrenExchange(Index1, Index2: Integer);

    procedure MoveTo(aToRecord: TGEDCOMRecord; aClearDest: Boolean = False); override;
    procedure Pack(); override;
    procedure ReplaceXRefs(aMap: TXRefReplaceMap); override;
    procedure ResetOwner(AOwner: TGEDCOMObject); override;
    procedure SaveToStream(AStream: System.IO.StreamWriter); override;
  ...
  strict private
    const
      PatriarchTag = '_PATRIARCH'; //zsv
      BookmarkTag = '_BOOKMARK'; //zsv
  ...
  private
    ...
    function GetSex: TGEDCOMObject.TGEDCOMSex;
    procedure SetSex(const Value: TGEDCOMObject.TGEDCOMSex);
  ...
  public
    ...
    function GetIndividualEvent(evName: string): TGEDCOMCustomEvent;
    ...
    property Restriction: TGEDCOMObject.TGEDCOMRestriction read GetRestriction write SetRestriction;
    property Sex: TGEDCOMObject.TGEDCOMSex read GetSex write SetSex;
  ...
  public
    ...
    procedure SetOriginatorArray(Value: array of string);
    procedure SetTitleArray(Value: array of string);
    procedure SetPublicationArray(Value: array of string);
    procedure SetTextArray(Value: array of string);
  ...
  public type
    TGEDCOMOrdinanceProcessFlag = (opNone, opYes, opNo);
  ...
  protected
    function GetDateTime: DateTime; virtual; abstract;
    procedure SetDateTime(Value: DateTime); virtual; abstract;
  protected
    procedure CreateObj(AOwner, AParent: TGEDCOMObject); override;
  public
    function ParseString(const AString: string): string; override;

    property Date: DateTime read GetDateTime write SetDateTime;
  end;
  ...
  public
    type
      ...
      TGEDCOMDateFormat = (dfGEDCOMStd, dfSystem);
    ...
    const
      GEDCOMMonthRusArray: array[1..12] of string =
        ('ЯНВ', 'ФЕВ', 'МАР', 'АПР', 'МАЙ', 'ИЮН',
         'ИЮЛ', 'АВГ', 'СЕН', 'ОКТ', 'НОЯ', 'ДЕК');

      GEDCOMMonthSysArray: array[1..12] of string =
        ('01.', '02.', '03.', '04.', '05.', '06.',
         '07.', '08.', '09.', '10.', '11.', '12.');

      GEDCOMMonthArray: array [1..12] of string =
        ('JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN',
         'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC');

      GEDCOMMonthFrenchArray: array [1..13] of string =
        ('VEND', 'BRUM', 'FRIM', 'NIVO', 'PLUV', 'VENT',
         'GERM', 'FLOR', 'PRAI', 'MESS', 'THER', 'FRUC', 'COMP');

      GEDCOMMonthHebrewArray: array [1..13] of string =
  ...
  protected
    ...
    function GetStringValue: string; override;
    function GetDateTime: DateTime; override;
    procedure SetDateTime(ADateTime: DateTime); override;
  ...
  public
    ...
    const
      GEDCOMDateRangeArray: array [TGEDCOMRange] of string =
        ('AFT', 'BEF', 'BET', 'AND');
  ...
  public
    ...
    const
      GEDCOMDateApproximatedArray: array[TGEDCOMApproximated] of string =
        ('', 'ABT', 'CAL', 'EST');
  ...
  private
    ...
    FSeconds: Word;
    FFraction: Word;
    function GetValue: TimeSpan;
    procedure SetValue(AValue: TimeSpan);
  protected
    procedure CreateObj(AOwner, AParent: TGEDCOMObject); override;
    function GetStringValue: string; override;
  public
    ...
    property Time: TimeSpan read GetValue write SetValue;
    property Value: TimeSpan read GetValue write SetValue;
  end;
  ...
  protected
    procedure CreateObj(AOwner, AParent: TGEDCOMObject); override;
    procedure SaveTagsToStream(AStream: System.IO.StreamWriter; const ATagSorting: array of string); override;
  public
    destructor Destroy; override;
...
begin
  ...
  for i := 0 to Length(FList) - 1 do
    if (TGEDCOMObject.CleanXRef(FList[i].oldXRef) = TGEDCOMObject.CleanXRef(oldXRef)) then begin
...
begin
  ...
  if (Length(Result) > 0) and (Result[1] = GEDCOMPointerDelimiter) then begin
    ...
    if (P > 0) then begin
      ...
      StrDelete(Result, 1, P + 1);
    end
    else
    if NoException
    then AXRef := ADefault
    else raise EGEDCOMException.Create(System.&String.Format(SXRefNotTerminated, [S]));
  end
  else
  if NoException
  then AXRef := ADefault
  else raise EGEDCOMException.Create(System.&String.Format(SXRefExpected, [S]));
end;

class function TGEDCOMObject.CleanXRef(const XRef: string): string;
begin
  Result := XRef;
  if (XRef = '') then Exit;
...
class function TGEDCOMObject.EncloseXRef(XRef: string): string;
begin
  if (XRef <> '') then begin
...
class function TGEDCOMObject.ExtractNumber(const S: string; var N: Integer;
...
begin
  Result := S;
  I := 0;
  while (I < Length(Result)) and TGKUtils.IsDigit(Result[I+1]) do
    Inc(I);

  if (I > 0) then begin
    N := Int32.Parse(Copy(Result, 1, I));
    StrDelete(Result, 1, I);
  end
  else
  if NoException
  then N := ADefault
  else raise EGEDCOMException.Create(System.&String.Format(SNotAValidNumber, [S]));
end;
...
function TGEDCOMTree.GetSignByRecord(aRecord: TGEDCOMRecord): string;
begin
...
begin
  ...
  I := 1;
  while FXRefIndex.IndexOf(sign + I.ToString()) >= 0 do Inc(I);
...
var
  fs: System.IO.StreamReader;
begin
  fs := System.IO.StreamReader.Create(aFileName, Encoding.GetEncoding(1251));
  try
    Clear;
    LoadFromStream(fs);
...
procedure TGEDCOMTree.LoadFromStream(AStream: System.IO.StreamReader);
  ...
  begin
    try
      ...
      on E: Exception do Trace.Write('Line '+LineNum.ToString()+'. Failed correct: ' + E.Message);
    end;
  end;
...
begin
  ...
  try
    ...
    I := -1;
    while not(AStream.Peek = -1) do begin
      S := AStream.ReadLine().Trim();
      Inc(I);

      // checkit
      if (I = 0) then begin
        // check for MyHeritage UTF-coded gedcom files
        if (S[1] = '0')
        then { normal gedcom }
        else StrDelete(S, 1, 3); { MyHeritage gedcom }
      end;
      ...
      // zsvfix
      if not(TGKUtils.IsDigit(S[1])) then begin
        LineCorrect(CurRecord, CurTag, I+1, S.Trim());
        Continue;
      end;
      // zsvfix-end
      ...
      try
        ...
        on E: EGEDCOMException do begin
          X := I+1;
          raise EGEDCOMException.Create('Syntax error in line '+X.ToString()+'.'#13+E.Message);
        end
        else raise;
      end;
...
procedure TGEDCOMTree.SaveToStream(AStream: System.IO.StreamWriter);
var
  I: Integer;
begin
...
procedure TGEDCOMTree.SaveHeaderToStream(AStream: System.IO.StreamWriter);
begin
  FHeader.SaveToStream(AStream);
end;

procedure TGEDCOMTree.SaveFooterToStream(AStream: System.IO.StreamWriter);
var
  S: string;
begin
  S := '0 TRLR';
  AStream.WriteLine(S{ + GEDCOMNewLine});
end;
...
procedure TGEDCOMList.SaveToStream(AStream: System.IO.StreamWriter);
var
  I: Integer;
begin
...
begin
  for i := FList.Count - 1 downto 0 do begin
    if (TObject(FList[i]) is TGEDCOMCustomTag) then begin
      ...
      if tag.IsEmpty() and tag.IsEmptySkip()
      then Delete(i);
    end;
  end;
end;
...
procedure TGEDCOMDate.SetDateTime(ADateTime: DateTime);
begin
  SetGregorian(ADateTime.Day, GEDCOMMonthArray[ADateTime.Month], ADateTime.Year);
end;

function TGEDCOMDate.GetDateTime: DateTime;
var
  M: Word;
begin
  ...
  else Result := DateTime.Create(FYear, M, FDay);
end;
...
begin
  ...
  if (I >= 1) and (I <= 2) then
  begin
    FDay := Int32.Parse(Copy(Result,1,I));
    StrDelete(Result, 1, I);
  end;
end;
...
begin
  ...
  case FDateCalendar of
    ...
    dcHebrew: begin
      ...
      if not(TGKUtils.IsDigit(Result[1])) then begin
        // gedcom standard date (01 JAN 1900)
        SU := Copy(Result, 1, 3).ToUpper();

        for I := 1 to 12 do
          if (GEDCOMMonthArray[I] = SU) or (DateTimeInfo.AbbreviatedMonthNames[I-1].ToUpper() = SU) then begin
...
begin
  ...
  if (I > 0) then begin
    FYear := Int32.Parse(Copy(Result, 1, I));
    StrDelete(Result, 1, I);

    if (Copy(Result, 1, 1) = GEDCOMYearModifierSeparator) and TGKUtils.IsDigits(Copy(Result,2,2)) then begin
      FYearModifier := Copy(Result, 2, 2);
      StrDelete(Result, 1, 3);
    end;

    if Copy(Result, 1, Length(GEDCOMYearBC)).ToUpper() = GEDCOMYearBC then begin
      FYearBC := True;
...
function TGEDCOMDate.StrToGEDCOMMonth(const S: string): string;
var
  SU: string;
  Month: Integer;
begin
  if Length(S) <> 3
  then raise EGEDCOMException.Create(System.&String.Format(SNotAValidMonth, [S]));
  ...
  raise EGEDCOMException.Create(System.&String.Format(SNotAValidMonth, [S]));
end;

function TGEDCOMDate.StrToGEDCOMMonthFrench(const S: string): string;
var
  SU: string;
  Month: Integer;
begin
  if Length(S) <> 4
  then raise EGEDCOMException.Create(System.&String.Format(SNotAValidFrenchMonth, [S]));
  ...
  raise EGEDCOMException.Create(System.&String.Format(SNotAValidFrenchMonth, [S]));
end;

function TGEDCOMDate.StrToGEDCOMMonthHebrew(const S: string): string;
var
  SU: string;
  Month: Integer;
begin
  if Length(S) <> 3
  then raise EGEDCOMException.Create(System.&String.Format(SNotAValidHebrewMonth, [S]));
  ...
  raise EGEDCOMException.Create(System.&String.Format(SNotAValidHebrewMonth, [S]));
end;

function TGEDCOMDate.IntToGEDCOMMonth(M: Word): string;
begin
  Result := GEDCOMMonthArray[M];
end;

function TGEDCOMDate.IntToGEDCOMMonthFrench(M: Word): string;
begin
  Result := GEDCOMMonthFrenchArray[M];
end;

function TGEDCOMDate.IntToGEDCOMMonthHebrew(M: Word): string;
begin
  Result := GEDCOMMonthHebrewArray[M];
end;

function TGEDCOMDate.GEDCOMMonthToInt(const S: string): Word;
var
  M: Word;
  SU: string;
begin
...
function TGEDCOMDate.GEDCOMMonthFrenchToInt(const S: string): Word;
var
  M: Word;
  SU: string;
begin
...
function TGEDCOMDate.GEDCOMMonthHebrewToInt(const S: string): Word;
var
  M: Word;
  SU: string;
begin
...
begin
  ...
  if (Copy(Result, 1, 3).ToUpper() = GEDCOMDateInterpretedIdent)
...
begin
  ...
  if Copy(Result, 1, 4).ToUpper() = GEDCOMDatePeriodFromIdent then begin
    StrDelete(Result, 1, 4);
  ...
  if Copy(Result, 1, 2).ToUpper() = GEDCOMDatePeriodToIdent then begin
    StrDelete(Result, 1, 2);
...
function TGEDCOMDatePeriod.GetDateTime: DateTime;
begin
  if FDateFrom.IsEmpty
  then Result := FDateTo.GetDateTime
  else
  if FDateTo.IsEmpty
  then Result := FDateFrom.GetDateTime
  else
...
procedure TGEDCOMDatePeriod.SetDateTime(ADateTime: DateTime);
begin
...
function TGEDCOMDateRange.ParseString(const S: string): string;
  ...
  begin
    ...
    or (SU = TGEDCOMDateApproximated.GEDCOMDateApproximatedArray[daAbout])
    or (SU = TGEDCOMDateApproximated.GEDCOMDateApproximatedArray[daCalculated])
    or (SU = TGEDCOMDateApproximated.GEDCOMDateApproximatedArray[daEstimated])
...
function TGEDCOMDateRange.GetDateTime: DateTime;
begin
...
procedure TGEDCOMDateRange.SetDateTime(ADateTime: DateTime);
begin
...
function TGEDCOMDatePhrase.GetDateTime: DateTime;
begin
  Result := DateTime.Create(0);
end;

procedure TGEDCOMDatePhrase.SetDateTime(ADateTime: DateTime);
var
  D: TGEDCOMDateExact;
begin
...
function TGEDCOMHeader.GetCharacterSet: TGEDCOMObject.TGEDCOMCharacterSet;
var
  S: string;
begin
  S := GetTagStringValue('CHAR').ToUpper();
...
procedure TGEDCOMHeader.SetCharacterSet(const Value: TGEDCOMObject.TGEDCOMCharacterSet);
var
  S: string;
begin
...
function TGEDCOMHeader.GetTransmissionDateTime: DateTime;
begin
  Result := TransmissionDate.Date.Add(TransmissionTime.Time);
end;

procedure TGEDCOMHeader.SetTransmissionDateTime(const Value: DateTime);
begin
  TransmissionDate.Date := Value.Date;
  TransmissionTime.Time := Value.TimeOfDay;
end;
...
procedure TGEDCOMCustomRecord.SaveValueToStream(AStream: System.IO.StreamWriter);
var
  S: string;
begin
  S := Level.ToString();
...
begin
  ...
  SU := Copy(S, 1, 3).ToUpper();
  if (SU = TGEDCOMDateApproximated.GEDCOMDateApproximatedArray[daAbout]) or
     (SU = TGEDCOMDateApproximated.GEDCOMDateApproximatedArray[daCalculated]) or
     (SU = TGEDCOMDateApproximated.GEDCOMDateApproximatedArray[daEstimated])
  then FValue := TGEDCOMDateApproximated.Create(Owner, Self)
  else
  if (SU = TGEDCOMDateInterpreted.GEDCOMDateInterpretedIdent)
  then FValue := TGEDCOMDateInterpreted.Create(Owner, Self)
  else
  if (SU = TGEDCOMDateRange.GEDCOMDateRangeArray[drAfter]) or
     (SU = TGEDCOMDateRange.GEDCOMDateRangeArray[drBefore]) or
     (SU = TGEDCOMDateRange.GEDCOMDateRangeArray[drBetween])
  then FValue := TGEDCOMDateRange.Create(Owner, Self)
  else
  if (Copy(S, 1, 4).ToUpper() = TGEDCOMDatePeriod.GEDCOMDatePeriodFromIdent) or
     (Copy(S, 1, 2).ToUpper() = TGEDCOMDatePeriod.GEDCOMDatePeriodToIdent)
...
function TGEDCOMDateValue.GetDateTime: DateTime;
begin
  if (FValue <> nil)
...
procedure TGEDCOMDateValue.SetDateTime(ADateTime: DateTime);
begin
  if (FValue <> nil)
...
procedure TGEDCOMCustomTag.SaveTagsToStream(AStream: System.IO.StreamWriter;
...
procedure TGEDCOMCustomTag.SaveTagToStream(AStream: System.IO.StreamWriter; ATag: TGEDCOMTag);
begin
  if (ATag <> nil)
  then ATag.SaveToStream(AStream);
end;

procedure TGEDCOMCustomTag.SaveTagToStream(AStream: System.IO.StreamWriter; const ATag: string);
var
  Tag: TGEDCOMTag;
  Index: Integer;
begin
...
procedure TGEDCOMCustomTag.SaveToStream(AStream: System.IO.StreamWriter);
begin
  SaveValueToStream(AStream);
  SaveTagsToStream(AStream, []);
end;

procedure TGEDCOMCustomTag.SaveValueToStream(AStream: System.IO.StreamWriter);
var
  S, Val: string;
begin
  S := Level.ToString() + GEDCOMDelimiter + Name;
...
procedure TGEDCOMCustomTag.SetTagIntegerValue(const ATag: string;
  AValue: Integer);
begin
  SetTagStringValue(ATag, AValue.ToString());
end;
...
begin
  ...
  then Result := AddTag(ATag, '', AClass)
    ...
    if (Tag is AClass)
    then Result := Tag
    else raise EGEDCOMException.Create(System.&String.Format(ATagHasInvalidType, [ATag, Tag.ClassName, AClass.ClassName]));
end;
...
procedure TGEDCOMCustomTag.SetTagStrings(ATag: TGEDCOMCustomTag; Value: array of string);
var
  I: Integer;
  S: string;
begin
...
function TGEDCOMCustomTag.IsEmptySkip(): Boolean;
begin
  Result := (GetTagProps(Name).EmptySkip);
end;

function TGEDCOMCustomTag.GetTagProps(const aName: string): TTagPropsRec;
var
  i: Integer;
begin
  for i := 1 to TagBaseSize - 1 do
...
begin
  inherited CreateObj(AOwner, AParent);
  FLists := TEnumSet.Create();
  FNotes := nil;
  FSourceCitations := nil;
  FMultimediaLinks := nil;
end;

procedure TGEDCOMTagWithLists.SetLists(ALists: TGEDCOMObject.TGEDCOMLists);
begin
  FLists := ALists;
end;
...
procedure TGEDCOMTagWithLists.SaveToStream(AStream: System.IO.StreamWriter);
begin
...
procedure TGEDCOMRecord.SetLists(ALists: TGEDCOMObject.TGEDCOMLists);
begin
  FLists := ALists;
end;
...
procedure TGEDCOMRecord.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
procedure TGEDCOMRecord.MoveTo(aToRecord: TGEDCOMRecord; aClearDest: Boolean = False);
var
  tag: TGEDCOMTag;
begin
  if (aClearDest)
  ...
  if (FTags <> nil) then begin
    while (FTags.Count > 0) do begin
      ...
      if (tag.Name = 'CHAN') and not(aClearDest)
      then tag.Destroy
      else begin
...
function TGEDCOMRecord.CreateUID(): string;
...
begin
  ...
  binary := id.ToByteArray();
  for i := 0 to High(binary) do begin
    val := Ord(binary[i]);
    checkA := checkA + val;
    checkB := checkB + checkA;
    Result := Result + System.&String.Format('{0:X2}', [val]);
  end;
  Result := Result + System.&String.Format('{0:X2}', [checkA]);
  Result := Result + System.&String.Format('{0:X2}', [checkB]);
end;
...
begin
  ...
  then Result := ''
    ...
    Result := System.&String.Format('{0:00}:{1:00}:{2:00}', [FHour, FMinutes, FSeconds]);
    if FFraction > 0 then
      Result := Result + '.' + FFraction.ToString();
  end;
end;

function TGEDCOMTime.GetValue: TimeSpan;
begin
  Result := TimeSpan.Create(0, FHour, FMinutes, FSeconds, 100*FFraction);
end;
...
procedure TGEDCOMTime.SetValue(AValue: TimeSpan);
var
  MSec: Word;
begin
  FHour := AValue.Hours;
  FMinutes := AValue.Minutes;
  FSeconds := AValue.Seconds;
  MSec := AValue.Milliseconds;
...
begin
  inherited CreateObj(AOwner, AParent);
  SetLists(TGEDCOMLists.Create([stNotes, stSource, stMultimedia]));
...
procedure TGEDCOMFamilyRecord.ChildrenExchange(Index1, Index2: Integer);
begin
  if (FChildren <> nil)
  then FChildren.Exchange(Index1, Index2);  
end;
...
function TGEDCOMFamilyRecord.GetRestriction: TGEDCOMObject.TGEDCOMRestriction;
var
  S: string;
begin
  S := GetTagStringValue('RESN').Trim().ToUpper();
  if (S = 'CONFIDENTIAL')
...
procedure TGEDCOMFamilyRecord.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
procedure TGEDCOMFamilyRecord.SetRestriction(const Value: TGEDCOMObject.TGEDCOMRestriction);
var
  S: string;
begin
  case Value of
...
procedure TGEDCOMFamilyRecord.MoveTo(aToRecord: TGEDCOMRecord; aClearDest: Boolean = False);
var
  toRec: TGEDCOMFamilyRecord;
  obj: TGEDCOMObject;
begin
  inherited MoveTo(aToRecord, aClearDest);
...
begin
  inherited CreateObj(AOwner, AParent);
  SetLists(TEnumSet.Create([stNotes, stSource, stMultimedia]));
...
function TGEDCOMIndividualRecord.GetIndividualEvent(evName: string): TGEDCOMCustomEvent;
var
  i: Integer;
...
begin
  ...
  for i := 0 to FIndividualEvents.Count - 1 do begin
    event := TGEDCOMCustomEvent(FIndividualEvents[i]);
...
function TGEDCOMIndividualRecord.GetRestriction: TGEDCOMObject.TGEDCOMRestriction;
var
  S: string;
begin
  S := GetTagStringValue('RESN').Trim().ToUpper();
...
function TGEDCOMIndividualRecord.GetSex: TGEDCOMObject.TGEDCOMSex;
var
  S: string;
begin
  S := GetTagStringValue('SEX').Trim().ToUpper();
...
function TGEDCOMIndividualRecord.IsLive(): Boolean;
begin
  Result := (GetIndividualEvent('DEAT') = nil);
end;

procedure TGEDCOMIndividualRecord.SetRestriction(const Value: TGEDCOMObject.TGEDCOMRestriction);
var
  S: string;
begin
  case Value of
...
procedure TGEDCOMIndividualRecord.SetSex(const Value: TGEDCOMObject.TGEDCOMSex);
var
  S: string;
begin
  case Value of
...
procedure TGEDCOMIndividualRecord.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
procedure TGEDCOMIndividualRecord.MoveTo(aToRecord: TGEDCOMRecord; aClearDest: Boolean = False);
var
...
begin
  ...
  if (FPersonalNames <> nil) and (aClearDest) then begin
...
begin
  inherited CreateObj(AOwner, AParent);
  SetLists(TEnumSet.Create([stNotes]));
...
var
  S: string;
begin
  S := GetTagStringValue('ORDI').Trim().ToUpper();
  if (S = 'YES')
  then Result := opYes
  else
  if S = 'NO'
...
begin
  inherited CreateObj(AOwner, AParent);
  SetLists(TEnumSet.Create([stNotes, stMultimedia]));
...
begin
  ...
  then raise EGEDCOMException.Create(System.&String.Format(EMaxLanguages, [GEDCOMMaxLanguages]))
  else
  if (Index >= 0) then begin
    if (FLanguages = nil)
    then FLanguages := TGEDCOMList.Create(Self);
...
begin
  inherited CreateObj(AOwner, AParent);
  SetLists(TEnumSet.Create([stNotes, stSource]));
...
procedure TGEDCOMMultimediaRecord.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
begin
  inherited CreateObj(AOwner, AParent);
  SetLists(TEnumSet.Create([stSource]));
  FRecordType := rtNote;
...
procedure TGEDCOMNoteRecord.MoveTo(aToRecord: TGEDCOMRecord; aClearDest: Boolean = False);
var
  cont: TStringList;
begin
...
procedure TGEDCOMSourceRecord.SetOriginatorArray(Value: array of string);
begin
  SetTagStrings(TagClass('AUTH', TGEDCOMTag), Value);
end;
...
procedure TGEDCOMSourceRecord.SetPublicationArray(Value: array of string);
begin
  SetTagStrings(TagClass('PUBL', TGEDCOMTag), Value);
end;
...
procedure TGEDCOMSourceRecord.SetTextArray(Value: array of string);
begin
  SetTagStrings(TagClass('TEXT', TGEDCOMTag), Value);
end;
...
procedure TGEDCOMSourceRecord.SetTitleArray(Value: array of string);
begin
  SetTagStrings(TagClass('TITL', TGEDCOMTag), Value);
end;
...
procedure TGEDCOMSourceRecord.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
  if Assigned(FRepositoryCitations) then FRepositoryCitations.SaveToStream(AStream);
end;

procedure TGEDCOMSourceRecord.MoveTo(aToRecord: TGEDCOMRecord; aClearDest: Boolean = False);
var
...
begin
  ...
  try
    titl.Text := (toSource.Title.Text + #10 + Self.Title.Text).Trim();
    orig.Text := (toSource.Originator.Text + #10 + Self.Originator.Text).Trim();
    publ.Text := (toSource.Publication.Text + #10 + Self.Publication.Text).Trim();
    text.Text := (toSource.Text.Text + #10 + Self.Text.Text).Trim();
...
function TGEDCOMChangeDate.GetChangeDateTime: DateTime;
begin
...
procedure TGEDCOMChangeDate.SetChangeDateTime(const Value: DateTime);
begin
  GetDate.Date := Value.Date;
  if (Value.TimeOfDay <> TimeSpan.Zero) then GetTime.Time := Value.TimeOfDay;
end;

function TGEDCOMChangeDate.ToString(): string;
var
  cdt: DateTime;
begin
  cdt := GetChangeDateTime();

  if (cdt.Ticks = 0)
  then Result := ''
  else Result := cdt.ToString('yyyy.MM.dd HH:mm:ss', nil);
end;
...
procedure TGEDCOMMultimediaLink.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
var
  S: string;
begin
  S := GetTagStringValue(MediaTypeTagName()).Trim().ToUpper();
...
var
  S: string;
begin
  S := GetTagStringValue('FORM').Trim().ToUpper();
...
var
  E: string;
begin
  E := System.IO.Path.GetExtension(AFile).ToLower();
...
procedure TGEDCOMAddress.SaveTagsToStream(AStream: System.IO.StreamWriter;
  const ATagSorting: array of string);
begin
...
begin
  ...
  then raise EGEDCOMException.Create(System.&String.Format(EMaxEmailAddresses, [GEDCOMMaxEmailAddresses]))
  else
  if (Index >= 0) then begin
    if (FEmailList = nil)
...
begin
  ...
  then raise EGEDCOMException.Create(System.&String.Format(EMaxFaxNumbers, [GEDCOMMaxFaxNumbers]))
  else
  if (Index >= 0) then begin
    if (FFaxList = nil)
...
begin
  ...
  then raise EGEDCOMException.Create(System.&String.Format(EMaxPhoneNumbers, [GEDCOMMaxPhoneNumbers]))
  else
  if (Index >= 0) then begin
    if (FPhoneList = nil)
...
begin
  ...
  then raise EGEDCOMException.Create(System.&String.Format(EMaxWebPages, [GEDCOMMaxWebPages]))
  else
  if (Index >= 0)
  then begin
    if (FWWWList = nil)
...
procedure TGEDCOMPersonalNamePieces.SaveToStream(AStream: System.IO.StreamWriter);
begin
  //inherited SaveToStream(AStream);
  //SaveValueToStream(AStream);
  SaveTagsToStream(AStream, []);
...
procedure TGEDCOMPersonalName.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
var
  S: string;
begin
  S := GetTagStringValue('TYPE').Trim().ToUpper();
...
var
  S: string;
begin
  S := GetTagStringValue('STAT').Trim().ToLower();
  if (S = 'challenged')
...
var
  S: string;
begin
  S := GetTagStringValue('PEDI').Trim().ToLower();
  if (S = 'adopted')
...
procedure TGEDCOMPlace.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
end;
...
function TGEDCOMEventDetail.GetRestriction: TGEDCOMObject.TGEDCOMRestriction;
var
  S: string;
begin
  S := GetTagStringValue('RESN').Trim().ToUpper();
...
procedure TGEDCOMEventDetail.SetRestriction(const Value: TGEDCOMObject.TGEDCOMRestriction);
var
  S: string;
begin
  case Value of
...
procedure TGEDCOMEventDetail.SaveToStream(AStream: System.IO.StreamWriter);
begin
  //inherited SaveToStream(AStream);
  //SaveValueToStream(AStream);
  SaveTagsToStream(AStream, []);
...
procedure TGEDCOMCustomEvent.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
var
  S: string;
begin
  S := GetTagStringValue('STAT').Trim().ToUpper();
  if (S = 'CHILD')
...
procedure TGEDCOMGroupRecord.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
procedure TGEDCOMResearchRecord.SaveToStream(AStream: System.IO.StreamWriter);
begin
  inherited SaveToStream(AStream);
...
var
  S: string;
begin
  S := GetTagStringValue('_PRIORITY').Trim().ToLower();
...
var
  S: string;
begin
  S := GetTagStringValue('_STATUS').Trim().ToLower();
...
var
  S: string;
begin
  S := GetTagStringValue('TYPE').Trim().ToLower();
