...
namespace GKCore
{
	public class TCalendarConv
	{
		...
		{
			return (a - b * (double)SysUtils.Floor((a / b)));
		}

		private int _modi(double a, double b)
		{
			return (int)SysUtils.Trunc(a - b * (double)SysUtils.Floor((a / b)));
		}
		...
		{
			int months = SysUtils.Floor(((double)(235 * year - 234) / 19.0));
			int parts = 12084 + 13753 * months;
			int day = months * 29 + SysUtils.Floor(((double)parts / 25920.0));
		...
		{
			return this._modi((double)SysUtils.Floor((j + 1.5)), 7.0);
		}
		...
		{
			double wjd = ((double)SysUtils.Floor((jd - 0.5)) + 0.5);
			double depoch = (wjd - 1721425.5);
			int quadricent = SysUtils.Floor((depoch / 146097.0));
			double dqc = this._modf(depoch, 146097.0);
			int cent = SysUtils.Floor((dqc / 36524.0));
			double dcent = this._modf(dqc, 36524.0);
			int quad = SysUtils.Floor((dcent / 1461.0));
			double dquad = this._modf(dcent, 1461.0);
			int yindex = SysUtils.Floor((dquad / 365.0));
			...
			month = SysUtils.Floor((((yearday + (double)leapadj) * 12.0 + 373.0) / 367.0));
			day = (int)(SysUtils.Trunc(wjd - this.gregorian_to_jd(year, month, 1)) + 1);
		}
		...
		{
			jd = (jd + 0.5);
			int b = SysUtils.Floor(jd) + 1524;
			int c = SysUtils.Floor((((double)b - 122.1) / 365.25));
			int d = SysUtils.Floor((365.25 * (double)c));
			int e = SysUtils.Floor(((double)(b - d) / 30.6001));
			month = SysUtils.Floor((double)this._if(e < 14, e - 1, e - 13));
			year = SysUtils.Floor((double)this._if(month > 2, c - 4716, c - 4715));
			day = b - d - SysUtils.Floor((30.6001 * (double)e));
			if (year < 1)
			{
				year--;
			}
		...
		{
			jd = ((double)SysUtils.Floor(jd) + 0.5);
			int count = SysUtils.Floor(((jd - 347995.5) * 98496.0 / 35975351.0));
			year = count - 1;
			int i = count;
			...
			day = (int)SysUtils.Trunc(jd - this.hebrew_to_jd(year, month, 1) + 1.0);
		}
		public double islamic_to_jd(int year, int month, int day)
		{
			return ((double)(day + SysUtils.Ceil((29.5 * (double)(month - 1))) + (year - 1) * 354 + SysUtils.Floor(((double)(3 + 11 * year) / 30.0))) + 1948439.5 - 1.0);
		}
		public void jd_to_islamic(double jd, ref int year, ref int month, ref int day)
		{
			jd = ((double)SysUtils.Floor(jd) + 0.5);
			year = SysUtils.Floor(((30.0 * (jd - 1948439.5) + 10646.0) / 10631.0));
			month = Math.Min(12, SysUtils.Ceil(((jd - (29.0 + this.islamic_to_jd(year, 1, 1))) / 29.5)) + 1);
			day = (int)SysUtils.Trunc(jd - this.islamic_to_jd(year, month, 1) + 1.0);
		}
		public double persian_to_jd(int year, int month, int day)
		{
			...
			return ((double)(day + this._if(month <= 7, (month - 1) * 31, (month - 1) * 30 + 6) + SysUtils.Floor(((epyear * 682.0 - 110.0) / 2816.0))) + (epyear - 1.0) * 365.0 + (double)(SysUtils.Floor((epbase / 2820.0)) * 1029983) + 1948319.5);
		}
		public void jd_to_persian(double jd, ref int year, ref int month, ref int day)
		{
			...
			int cycle = SysUtils.Floor((depoch / 1029983.0));
			...
			{
				int aux = SysUtils.Floor(((double)cyear / 366.0));
				int aux2 = this._modi((double)cyear, 366.0);
				ycycle = SysUtils.Floor(((double)(2134 * aux + 2816 * aux2 + 2815) / 1028522.0)) + aux + 1;
			}
			...
			month = this._if(yday <= (double)186f, SysUtils.Ceil((yday / 31.0)), SysUtils.Ceil(((yday - 6.0) / 30.0)));
			day = (int)SysUtils.Trunc(jd - this.persian_to_jd(year, month, 1) + 1.0);
		}
		...
		public void jd_to_indian_civil(double jd, ref int year, ref int month, ref int day)
		{
			...
			{
				month = 1;
				day = SysUtils.Floor((yday + 1.0));
			}
			else
			{
				int mday = SysUtils.Floor((yday - (double)Caitra));
				if (mday < 155)
				{
					month = SysUtils.Floor(((double)mday / 31.0)) + 2;
					day = mday % 31 + 1;
				}
				else
				{
					mday -= 155;
					month = SysUtils.Floor(((double)mday / 30.0)) + 7;
					day = mday % 30 + 1;
				}
			}
		}
		...
		public void jd_to_bahai(double jd, ref int major, ref int cycle, ref int year, ref int month, ref int day)
		{
			...
			major = SysUtils.Floor(((double)bys / 361.0)) + 1;
			cycle = SysUtils.Floor((this._modf((double)bys, 361.0) / 19.0)) + 1;
			...
			month = this._if(jd >= bld, 20, SysUtils.Floor((days / 19.0)) + 1);
			day = SysUtils.Floor((jd + 1.0 - this.bahai_to_jd(major, cycle, year, month, 1)));
		}
