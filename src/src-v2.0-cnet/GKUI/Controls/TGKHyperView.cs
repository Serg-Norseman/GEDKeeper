...
namespace GKUI.Controls
{
	public class TGKHyperView : Panel, IDisposable
	{
		...
		public TGKHyperView.TRuleStyle RuleStyle
		{
			get
			{
				return this.FRuleStyle;
			}
			set
			{
				this.SetRuleStyle(value);
			}
		}
		...
		private void LinesChanged(object Sender)
		{
			if ((int)this.FHeightCount != this.FLines.Count)
			{
				...
				int[] arg_35_0 = this.FHeights;
				int num = (int)this.FHeightCount;
				int[] array = arg_35_0;
				int arg_3D_0;
				if ((arg_3D_0 = num) < 0)
				{
				...
				int[] array2;
				int[] expr_42 = array2 = new int[arg_3D_0];
				if (num > 0 && array != null)
				{
					int num2;
		...
		{
			string ss = BDSSystem.WStrFromWChar(s[i - 1]);
			while (s[i + 1 - 1] != '~')
			{
				i++;
				ss += s[i - 1];
		...
		{
			if (y >= -hMax && BDSSystem.WStrCmp(ss, "") != 0)
			{
				if (!resize)
				{
					grx.DrawString(ss, this.FTextFont, this.FDefBrush, ((float)((double)x)), ((float)((double)y)));
				}
		...
		private void GotoLink(ushort Link)
		{
			...
			{
				num++;
				while (BDSSystem.Pos(i, this.FLines[j]) <= 0)
				{
					h += this.FHeights[j];
					j++;
					if (j == num)
		...
		{
			if (this.FPageWidth < base.ClientRectangle.Width)
			{
				this.FRange.X = 0;
				this.LeftPos = 0;
			}
			...
			{
				this.FRange.X = this.FPageWidth - base.ClientRectangle.Width;
			}
			VCLUtils.SetScrollRange((uint)this.Handle.ToInt32(), 0, 0, this.FRange.X, (LongBool)0);
			this.FRange.Y = this.FPageHeight;
				...
				if (msg != 135)
				{
					if (msg != 276)
					{
						if (msg == 277)
						{
							...
							switch (TGKSys.GetScrollEventType(wParam & 65535u))
							{
								...
								case ScrollEventType.ThumbTrack:
								{
									...
									VCLUtils.GetScrollInfo((uint)this.Handle.ToInt32(), 1, ref ScrollInfo);
					...
					else
					{
						...
						switch (TGKSys.GetScrollEventType(wParam & 65535u))
						{
							...
							case ScrollEventType.ThumbTrack:
							{
								...
								ScrollInfo.cbSize = (uint)Marshal.SizeOf( ScrollInfo );
								ScrollInfo.fMask = 23u;
								VCLUtils.GetScrollInfo((uint)this.Handle.ToInt32(), 0, ref ScrollInfo);
		...
		public TGKHyperView()
		{
			...
			this.FLines.set_OnChange(new TNotifyEvent(this.LinesChanged));
