...
namespace GKUI.Controls
{
	public class TGKListView : ListView
	{
		...
		{
			internal int FSortColumn;
			internal SortOrder FSortOrder;
			internal CaseInsensitiveComparer FObjCompare;
			...
			int IComparer.Compare(object x, object y)
			{
				...
				int comp_res = TGKSys.agCompare(item.SubItems[this.FSortColumn].Text, item2.SubItems[this.FSortColumn].Text);
				int Result;
		...
		private TGKListView.TListViewColumnSorter lvwColumnSorter;
		...
		{
			if (aColumnIndex >= 0) {
				this.Columns[aColumnIndex].AutoResize(ColumnHeaderAutoResizeStyle.ColumnContent);
			}
		}
	}
}
