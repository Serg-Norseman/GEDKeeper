...
{
	public class TMapBrowser : WebBrowser, IDisposable
	{
		public class TGMapPoint
		{
			internal DateTime FDate;
			internal double FLatitude;
			internal double FLongitude;
			internal string FHint;
		...
		public TMapBrowser.TGMapPoint this[int Index]
		{
		...
		private void gm_ExecScript(string Script)
		{
			if (Script.Trim() != "")
			{
				...
				{
					TGKSys.LogWrite("TMapBrowser.gm_ExecScript(): " + E.Message);
				}
			}
		}
		...
		private static string FillNode(XmlNode Node)
		{
			...
			if (Node.ChildNodes != null && Node.ChildNodes.Count > 0)
			{
				...
				if (num >= i)
				{
					...
					while (true)
					{
						TMapBrowser.FillNode(Node.ChildNodes[i]);
		...
		private TMapBrowser.TGMapPoint GetMapPoint(int Index)
		{
			TMapBrowser.TGMapPoint Result = null;
			if (Index >= 0 && Index < this.FMapPoints.Count)
			{
				Result = (this.FMapPoints[Index] as TMapBrowser.TGMapPoint);
			}
			return Result;
		}
		...
		private TMapBrowser.TCoordsRect GetPointsFrame()
		{
			TMapBrowser.TCoordsRect Result = new TCoordsRect();
			if (this.FMapPoints.Count > 0)
			{
				TMapBrowser.TGMapPoint pt = this.FMapPoints[0] as TMapBrowser.TGMapPoint;
		...
		{
			TMapBrowser.TGMapPoint pt = new TMapBrowser.TGMapPoint(aLatitude, aLongitude, aHint);
		...
		public void InitMap()
		{
			this.FMapFile = TGKSys.GetTempDir() + "\\GEDKeeperMap.html";
			StreamWriter tf = new StreamWriter(this.FMapFile, false, Encoding.UTF8);
			tf.WriteLine(TMapBrowser.MapContent);
			tf.Close();
		...
		public void RefreshPoints()
		{
			...
			if (this.MapPointsCount > 0)
			{
				...
				{
					TMapBrowser.TGMapPoint pt = this[i];
		...
		{
			TMapBrowser.TCoordsRect rt = this.GetPointsFrame();
		...
		{
			TMapBrowser.xmlDocument = new XmlDocument();
		}

		public static void GeoDone()
		{
			TMapBrowser.xmlDocument = null;
		}
		...
		public static bool GetInetFile([In] string FileURL, ref Stream Stream)
		{
			...
			try
			{
				...
				TProxy proxy = GKL.fmGEDKeeper.Options.Proxy;
				if (proxy.UseProxy)
				{
			...
			{
				TGKSys.LogWrite("GetInetFile(): " + E.Message);
				Stream = null;
				Result = false;
			}
			return Result;
		...
		public static void RequestGeoCoords(string SearchString, TObjectList aPoints)
		{
			if (aPoints != null)
			{
				Stream stm = null;
				try
				{
					SearchString = VCLUtils.StringReplace(SearchString.Trim(), " ", "+", TReplaceFlags.rfReplaceAll);
					...
					if (TMapBrowser.GetInetFile(FileOnNet, ref stm))
					{
						TMapBrowser.xmlDocument.Load(stm);
						XmlNode Node = TMapBrowser.xmlDocument.DocumentElement;
						Node = Node["Response"];
						if (Node != null && Node.ChildNodes.Count > 0)
						{
							...
							if (num >= i)
							{
								...
								do
								{
									if (BDSSystem.WStrCmp(Node.ChildNodes[i].Name, "Placemark") == 0)
									{
										...
										if (AddressNode != null && PointNode != null)
										{
											...
											{
												pt.Longitude = TGKSys.StrToFloatDef(coord_tokens[0], -1.0);
												pt.Latitude = TGKSys.StrToFloatDef(coord_tokens[1], -1.0);
		...
		public static string RequestGeoAddress(double aLatitude, double aLongitude)
		{
			...
			try
			{
				...
				{
					TMapBrowser.xmlDocument.Load(stm);
					Result = TMapBrowser.FillNode(TMapBrowser.xmlDocument.DocumentElement);
				}
			}
			finally
			{
