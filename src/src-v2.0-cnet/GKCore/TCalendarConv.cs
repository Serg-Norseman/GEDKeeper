...
namespace GKCore
{
	public class TCalendarConv
	{
		...
		internal double _modf(double a, double b)
		{
			return (a - b * (double)VCLUtils.Floor((a / b)));
		}
		internal int _modi(double a, double b)
		{
			return (int)BDSSystem.Trunc(a - b * (double)VCLUtils.Floor((a / b)));
		}
		internal int _if(bool aCond, int aThen, int aElse)
		{
			int Result;
			if (aCond)
			{
		...
		internal bool hebrew_leap(int year)
		{
			return this._modf((double)(year * 7 + 1), 19.0) < (double)7f;
		}
		internal int hebrew_year_months(int year)
		{
			return this._if(this.hebrew_leap(year), 13, 12);
		}
		internal int hebrew_delay_1(int year)
		{
			int months = VCLUtils.Floor(((double)(235 * year - 234) / 19.0));
			int parts = 12084 + 13753 * months;
			int day = months * 29 + VCLUtils.Floor(((double)parts / 25920.0));
		...
		internal int hebrew_delay_2(int year)
		{
		...
		internal double hebrew_year_days(int year)
		{
			return (this.hebrew_to_jd(year + 1, 7, 1) - this.hebrew_to_jd(year, 7, 1));
		}
		internal int hebrew_month_days(int year, int month)
		{
			int Result;
		...
		internal bool leap_gregorian(int year)
		{
			return year % 4 == 0 && (year % 100 != 0 || year % 400 == 0);
		}
		internal bool leap_julian(int year)
		{
			return this._modf((double)year, 4.0) == (double)this._if(year > 0, 0, 3);
		}
		internal bool leap_islamic(int year)
		{
			return (year * 11 + 14) % 30 < 11;
		}
		internal bool leap_persian(int year)
		{
			return ((year - this._if(year > 0, 474, 473)) % 2820 + 474 + 38) * 682 % 2816 < 682;
		}
		public int jwday(double j)
		{
			return this._modi((double)VCLUtils.Floor((j + 1.5)), 7.0);
		}
		...
		{
			double wjd = ((double)VCLUtils.Floor((jd - 0.5)) + 0.5);
			double depoch = (wjd - 1721425.5);
			int quadricent = VCLUtils.Floor((depoch / 146097.0));
			double dqc = this._modf(depoch, 146097.0);
			int cent = VCLUtils.Floor((dqc / 36524.0));
			double dcent = this._modf(dqc, 36524.0);
			int quad = VCLUtils.Floor((dcent / 1461.0));
			double dquad = this._modf(dcent, 1461.0);
			int yindex = VCLUtils.Floor((dquad / 365.0));
			...
			month = VCLUtils.Floor((((yearday + (double)leapadj) * 12.0 + 373.0) / 367.0));
			day = (int)(BDSSystem.Trunc(wjd - this.gregorian_to_jd(year, month, 1)) + (long)((ulong)1));
		}
		...
		{
			jd = (jd + 0.5);
			int b = VCLUtils.Floor(jd) + 1524;
			int c = VCLUtils.Floor((((double)b - 122.1) / 365.25));
			int d = VCLUtils.Floor((365.25 * (double)c));
			int e = VCLUtils.Floor(((double)(b - d) / 30.6001));
			month = VCLUtils.Floor((double)this._if(e < 14, e - 1, e - 13));
			year = VCLUtils.Floor((double)this._if(month > 2, c - 4716, c - 4715));
			day = b - d - VCLUtils.Floor((30.6001 * (double)e));
			if (year < 1)
			{
				year--;
			}
		...
		{
			jd = ((double)VCLUtils.Floor(jd) + 0.5);
			int count = VCLUtils.Floor(((jd - 347995.5) * 98496.0 / 35975351.0));
			year = count - 1;
			int i = count;
			...
			day = (int)BDSSystem.Trunc(jd - this.hebrew_to_jd(year, month, 1) + 1.0);
		}
		public double islamic_to_jd(int year, int month, int day)
		{
			return ((double)(day + VCLUtils.Ceil((29.5 * (double)(month - 1))) + (year - 1) * 354 + VCLUtils.Floor(((double)(3 + 11 * year) / 30.0))) + 1948439.5 - 1.0);
		}
		public void jd_to_islamic(double jd, ref int year, ref int month, ref int day)
		{
			jd = ((double)VCLUtils.Floor(jd) + 0.5);
			year = VCLUtils.Floor(((30.0 * (jd - 1948439.5) + 10646.0) / 10631.0));
			month = Math.Min(12, VCLUtils.Ceil(((jd - (29.0 + this.islamic_to_jd(year, 1, 1))) / 29.5)) + 1);
			day = (int)BDSSystem.Trunc(jd - this.islamic_to_jd(year, month, 1) + 1.0);
		}
		public double persian_to_jd(int year, int month, int day)
		{
			...
			return ((double)(day + this._if(month <= 7, (month - 1) * 31, (month - 1) * 30 + 6) + VCLUtils.Floor(((epyear * 682.0 - 110.0) / 2816.0))) + (epyear - 1.0) * 365.0 + (double)(VCLUtils.Floor((epbase / 2820.0)) * 1029983) + 1948319.5);
		}
		public void jd_to_persian(double jd, ref int year, ref int month, ref int day)
		{
			...
			int cycle = VCLUtils.Floor((depoch / 1029983.0));
			...
			{
				int aux = VCLUtils.Floor(((double)cyear / 366.0));
				int aux2 = this._modi((double)cyear, 366.0);
				ycycle = VCLUtils.Floor(((double)(2134 * aux + 2816 * aux2 + 2815) / 1028522.0)) + aux + 1;
			}
			...
			month = this._if(yday <= (double)186f, VCLUtils.Ceil((yday / 31.0)), VCLUtils.Ceil(((yday - 6.0) / 30.0)));
			day = (int)BDSSystem.Trunc(jd - this.persian_to_jd(year, month, 1) + 1.0);
		}
		...
		public void jd_to_indian_civil(double jd, ref int year, ref int month, ref int day)
		{
			...
			{
				month = 1;
				day = VCLUtils.Floor((yday + 1.0));
			}
			else
			{
				int mday = VCLUtils.Floor((yday - (double)Caitra));
				if (mday < 155)
				{
					month = VCLUtils.Floor(((double)mday / 31.0)) + 2;
					day = mday % 31 + 1;
				}
				else
				{
					mday -= 155;
					month = VCLUtils.Floor(((double)mday / 30.0)) + 7;
					day = mday % 30 + 1;
				}
			}
		}
		...
		public void jd_to_bahai(double jd, ref int major, ref int cycle, ref int year, ref int month, ref int day)
		{
			...
			major = VCLUtils.Floor(((double)bys / 361.0)) + 1;
			cycle = VCLUtils.Floor((this._modf((double)bys, 361.0) / 19.0)) + 1;
			...
			month = this._if(jd >= bld, 20, VCLUtils.Floor((days / 19.0)) + 1);
			day = VCLUtils.Floor((jd + 1.0 - this.bahai_to_jd(major, cycle, year, month, 1)));
		}
		public string date_to_str(int aYear, int aMonth, int aDay, TCalendarConv.TDateEra aEra)
		{
			...
			if (aEra != TCalendarConv.TDateEra.AD)
			{
				Result += " до н.э.";
			}
			return Result;
		...
		{
			TCalendarConv.BahaiWeekdays = new string[]
			{
				"Джамаль", 
				"Камаль", 
				"Фидаль", 
			...
			TCalendarConv.BahaiMonths = new string[]
			{
				"Бахa", 
				"Джалaл", 
				"Джамaл", 
			...
			TCalendarConv.IndianCivilWeekdays = new string[]
			{
				"равивар", 
				"сомвар", 
				"мангалвар", 
			...
			TCalendarConv.IndianCivilMonths = new string[]
			{
				"Чайтра", 
				"Ваисакха", 
				"Джанштха", 
			...
			TCalendarConv.PersianWeekdays = new string[]
			{
				"йекшанбе", 
				"душанбе", 
				"сешанбе", 
			...
			TCalendarConv.PersianMonths = new string[]
			{
				"Фарвардин", 
				"Ордибехешт", 
				"Хордад", 
			...
			TCalendarConv.IslamicWeekdays = new string[]
			{
				"аль-ахад", 
				"аль-иснайн", 
				"ас-саласа'", 
			...
			TCalendarConv.IslamicMonths = new string[]
			{
				"мухаррам", 
				"сафар", 
				"рабии`у ль-авваль", 
			...
			TCalendarConv.HebrewWeekdays = new string[]
			{
				"алеф", 
				"бейт", 
				"гимел", 
			...
			TCalendarConv.HebrewMonths = new string[]
			{
				"Нисан", 
				"Ияр", 
				"Сиван", 
