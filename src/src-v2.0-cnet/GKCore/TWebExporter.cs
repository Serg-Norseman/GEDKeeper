...
namespace GKCore
{
	public class TWebExporter : THTMLExporter
	{
		...
		private void Step(TObjectList aTable, int row_index, int col_index, TTreeCell prev, TGEDCOMIndividualRecord cur)
		{
			if (cur != null)
			{
				...
				if (cur.ChildToFamilyLinksCount > 0)
				{
					TGEDCOMFamilyRecord family = cur.GetChildToFamilyLink(0).Family;
					...
					if (iFather != null || iMother != null)
					{
						...
						this.Step(aTable, row_index, col_index + 1, cur_cell, iFather);

						row_index = aTable.IndexOf(row);
						if (iMother != null) this.AddRow(aTable, row_index + 1, col_index + 1);
						this.Step(aTable, row_index + 2, col_index + 1, cur_cell, iMother);
					}
				}
		...
		private void GenTree(StreamWriter aStream, TGEDCOMIndividualRecord iRec)
		{
			try
			{
				...
				try
				{
					this.Step(table_rows, 0, 0, null, iRec);
					...
					do
					{
						...
						if (rec is TGEDCOMIndividualRecord)
						{
							...
							else
							{
								ushort j = 0;
								ushort d = 0;
								TGenEngine.GetIndependentDate(ev.Detail.Date.Value, ref year, ref j, ref d);
								if (year == 0)
								{
									year = -1;
								}
			...
			try
			{
				...
				if (num >= i)
				{
					...
					do
					{
						...
						if (rec is TGEDCOMIndividualRecord)
						{
							...
							string nam = "";
							string pat = "";
							TGenEngine.GetNameParts(ind, ref fam, ref nam, ref pat);
							if (nam == "")
							{
								if (unk == null)
								{
				...
				do
				{
					if (lat[(int)c - 65] != null)
					{
						...
						{
							index_str, 
							"<a href=\"index_names.htm#", 
							TGKSys.NumUpdate((int)c, 3), 
							"\">", 
							BDSSystem.WStrFromWChar(c), 
							"</a>&nbsp;"
						});
				...
				do
				{
					if (rus[(int)c - 1040] != null)
					{
						index_str = index_str + "<a href=\"index_names.htm#" + TGKSys.NumUpdate((int)c, 3) + "\">" + c + "</a>&nbsp;";

						fs_names.WriteLine("<li><a name=\"" + TGKSys.NumUpdate((int)c, 3) + "\"><b>" + c + "</b></a>");
				...
				if (unk != null)
				{
					index_str = index_str + "<a href=\"index_names.htm#" + TGKSys.NumUpdate(63, 3) + "\">?</a>&nbsp;";
					fs_names.WriteLine(string.Concat(new string[]
					{
						"<li><a name=\"", TGKSys.NumUpdate(63, 3), "\"><b>", "?", "</b></a>"
					}));
		...
		private void WriteFamilyIndex(StreamWriter aStream)
		{
			...
			try
			{
				...
				if (unk != null)
				{
					index_str = index_str + "<a href=\"index_surnames.htm#" + TGKSys.NumUpdate(63, 3) + "\">?</a>" + "&nbsp;";
		...
		private void WriteSurnames(StreamWriter aStream, char aSym, TStringList aNames)
		{
			...
			{
				"<li><a name=\"", 
				TGKSys.NumUpdate((int)aSym, 3), 
				"\"><b>", 
				BDSSystem.WStrFromWChar(aSym), 
				"</b></a><ul>"
			}));
			...
			try
			{
				...
				if (num >= i)
				{
					...
					do
					{
						...
						string j = "";
						string p = "";
						TGenEngine.GetNameParts(i_rec, ref f, ref j, ref p);
						f = TGenEngine.PrepareRusFamily(f, i_rec.Sex == TGEDCOMObject.TGEDCOMSex.svFemale);
		...
		public override void Generate()
		{
			...
			TGKSys.LoadExtFile(this.FPath + "index.htm");
		}
